...
function Select({ label, icon, from, to, value, onChange, onReset, options, placeholder }: {
  label: string; icon: IconName; from: string; to: string;
    ...
    <div className="relative z-20 shrink-0 border-b border-slate-200 bg-white">
      <div className="px-4 py-2.5">
        ...
        <div className="grid grid-cols-1 items-end gap-2 sm:grid-cols-2 lg:grid-cols-5">
          <Select label="Province" icon="map-pin" from="#19c2b1" to="#0d9488" placeholder="Toutes"
          ...
          <Select label="Antenne" icon="tower" from="#36b3ec" to="#0093d5" placeholder="Toutes"
            value={f.antenne} onChange={(v) => f.set({ antenne: v, zone: null, aire: null })} onReset={() => f.resetField("antenne")} options={geo.antennes} />
          <Select label="Zone de santé" icon="building" from="#9d5cf5" to="#7c3aed" placeholder="Toutes"
            value={f.zone} onChange={(v) => f.set({ zone: v, aire: null })} onReset={() => f.resetField("zone")} options={geo.zones} />
          <Select label="Aire de santé" icon="clinic" from="#2bbd6b" to="#1f9d57" placeholder="Toutes"
