...
interface SubTab { href: string; label: string }
interface MainTab { key: string; label: string; icon: IconName; href?: string; subs?: SubTab[] }

const NAV: MainTab[] = [
  {
    key: "sav",
    label: "Récupération des enfants ZD & SV",
    icon: "syringe",
    subs: [
      { href: "/", label: "Synthèse & vue d'ensemble" },
      { href: "/identification-cs", label: "Identification au centre de santé (IT)" },
      { href: "/identification-relais", label: "Identification communautaire (relais)" },
      { href: "/planification", label: "Planification de la récupération" },
      { href: "/resultats", label: "Résultats de la récupération" },
    ],
  },
  { key: "superv", label: "Supervision des équipes", icon: "shield", href: "/supervision" },
  ...
  return (
    <aside className="hidden h-screen w-64 shrink-0 flex-col bg-navy-800 text-white lg:flex">
      <div className="border-b border-white/10 px-5 pt-5 pb-4">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/55">Tableau de bord SAV</p>
        <p className="mt-0.5 text-[20px] font-extrabold leading-tight">Tshuapa</p>
        <p className="mt-0.5 text-[11.5px] font-semibold leading-tight text-oms-300">Semaine Africaine de la Vaccination</p>
        <p className="mt-2.5 inline-flex items-center gap-1.5 text-[10px] text-white/55">
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-good-500" /> Récupération enfants ZD &amp; SV
        </p>
      </div>
