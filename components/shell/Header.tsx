...
import { triggerRefresh, useSav } from "@/lib/client/api";
import { fmtDateTime } from "@/lib/client/format";

const TITLES: Record<string, string> = {
  "/": "Synthèse — Récupération des enfants ZD & SV",
  "/identification-cs": "Identification au centre de santé (IT)",
  "/identification-relais": "Identification communautaire par les relais",
  "/planification": "Planification de la récupération des enfants",
  "/resultats": "Résultats de la récupération des enfants",
  "/supervision": "Supervision des équipes de vaccination",
  "/telecharger-rapport": "Télécharger Rapport — Génération automatique (PPTX)",
};

export default function Header() {
  const path = usePathname();
  const title = TITLES[path] ?? "SAV Tshuapa — Récupération des enfants";
  const { data, refresh, isValidating } = useSav();
    ...
    <header className="h-[68px] shrink-0 px-3 md:px-5 flex items-center gap-3 md:gap-4 bg-navy-700 text-white" style={{ borderBottom: "3px solid #0093d5" }}>
      ...
      <div className="flex-1 min-w-0 text-center px-2">
        <div className="hidden sm:block text-[9px] md:text-[10px] uppercase tracking-[0.2em] text-white/65 font-semibold truncate">
          Semaine Africaine de la Vaccination · PEV-Central / OMS — Tshuapa, RDC
        </div>
      ...
      <div className="hidden lg:flex items-center gap-3 text-[10px] text-right">
        <div className="leading-tight">
          <div className="uppercase tracking-wider text-white/55">{data?.meta.live ? "Synchro Kobo" : "Données figées"}</div>
